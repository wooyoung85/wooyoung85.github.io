...
## 13. Build System
- 종속성 관리를 지원하는 Build System을 선택하는 것을 강력하게 추천하고 이런 Build System은 "Maven Central" 저장소에 게시된 아티팩트를 사용할 수 있다.
- Maven이나 Gradle을 선택하는걸 추천
...
- curated list는 잘 정리된 제3자 라이브러리들 뿐만 아니라 스프링 부트에서 사용할 수 있는 모든 스프링 모듈들이 들어있고 Maven과 Gradle에서 사용되어지는 표준 Bills of Materials(BOM)으로 제공된다.
    > 스프링 부트의 각 릴리즈 버전은 스프링 프레임워크의 기본 버전과 연관되기 때문에 버전을 지정하지 않는 것이 좋다.
...
- 모든 사람이 `spring-boot-starter-parent` POM을 상속하기 원하지는 않는다.<br/>
회사 표준을 사용해야만 하거나 모든 메이븐 설정을 명시적으로 선언하는 걸 선호할 수도 있음
...
- 스프링 부트는 프로젝트를 실행가능한 jar 파일로 패키지 할 수 있는 메이븐 플러그인을 포함한다.
    ...
    ```
    > 만약 Spring Boot starter parent pom을 사용했다면 사용자는 오직 플러그인을 추가만 하면 된다. parent에 정의된 설정을 변경하는게 아니라면 따로 구성을 할 필요가 없다.

## 13.5 Starters
- Starters는 당신의 어플리케이션에 포함할 수 있는 편리한 종속성 기술서들의 모임입니다.
...
> - 모든 공식적인 starters는 유사한 패턴을 따른다; `spring-boot-starter-*` `*` 위치는 어플리케이션의 특정 타입임
> - naming 구조는 starter를 찾을 때 도움이 될 수 있도록 의도한 것이다.
> - 많은 IDE들의 메이븐 integration은 이름으로 종속성을 검색할 수 있게 해준다.  
예를 들어, 적절한 Eclipse 또는 STS 플러그인이 설치된 상태에서 POM 에디터에서  `ctrl-space` 을 누르고 전체목록을 보려면 "spring-boot-starter"를 입력할 수 있다.
...
## 14. 코드 구조화
스프링 부트는 특정한 코드 레이아웃이 요구되지 않는다. 그러나 도움이 되는 몇 가지 모범 사례가 있습니다.
